...
fork：系统调用通过发起系统调用执行 do_fork 函数，用于创建并唤醒一个新线程。具体过程包括初始化一个新线程结构，分配内核栈空间，并为新线程分配新的虚拟内存或与其他线程共享现有的虚拟内存。接着，系统会复制原线程的上下文与中断帧，设置新线程的上下文与中断帧，然后将新线程插入到哈希表和调度链表中，最后唤醒新线程并返回其线程ID。

exec：系统调用通过发起系统调用执行 do_execve 函数，用于在用户空间创建并加载用户程序，从而替换当前线程的执行映像。执行过程中，首先会回收当前线程的虚拟内存空间，释放其占用的资源。随后，为当前线程分配新的虚拟内存空间，并将指定的用户程序加载到新的虚拟内存中，准备执行新的应用程序。

wait：系统调用通过发起系统调用执行 do_wait 函数，用于等待子线程完成执行。当调用 wait 时，系统会查找当前线程的子线程中状态为 PROC_ZOMBIE 的线程。如果找到这样的子线程，系统会设置当前线程的状态并进行线程切换；如果子线程已经完全退出，则调用 do_exit 进行清理。随后，系统会将已完成的子线程从哈希表和调度链表中删除，并释放其占用的所有资源。

exit：调用通过发起系统调用执行 do_exit 函数，用于终止当前线程的执行并进行必要的清理工作。在执行过程中，系统首先检查当前线程的虚拟内存是否被其他线程共享，如果没有，则销毁该虚拟内存空间。接着，将当前线程的状态设为 PROC_ZOMBIE，表示线程已终止但尚未被回收，并唤醒父线程以通知其子线程已退出。最后，调用调度器（schedule）切换到其他可运行的线程执行。
